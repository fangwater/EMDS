...
public:
    ...
    std::shared_ptr<ContractBufferMap<T,EXCHANGE::SZ>> sz_buffer_map;
    //collector
...
public:
    ...
    void register_collect_target_buffer_map(
        const std::shared_ptr<ContractBufferMap<T,EXCHANGE::SH>>& sh_buffer_map_sp,
        const std::shared_ptr<ContractBufferMap<T,EXCHANGE::SZ>>& sz_buffer_map_sp
    ){
        //buffer不需要日志, 其职责仅为接收来自缓存的插入
        sh_buffer_map = sh_buffer_map_sp;
        sz_buffer_map = sz_buffer_map_sp;
    }
    virtual void init(const std::shared_ptr<LoggerManager> &logger_manager){
        //CollectorManager 需要输出日志
        logger = logger_manager->get_logger(fmt::format("CollectorManager_{}", str_type_info<T>()));
        logger->info(fmt::format("Create CollectorManager_{} at {}", str_type_info<T>(),get_current_time_as_string()));
        ...
        sz_period_computer->init_per_period_ctx();
